 ...
 * consecutiva. (Nota: recalco lo de modificar y lo de añadir porque simplemente coger el 1º elemento y hacer una nueva lista
 * hasta el último)
 * 
 ...
 * ¡El primer parámetro tiene un valor menor que el segundo parámetro!
 * 
 * Ejemplo:
 * 
 * menor = 3 
...
public class ListaDeEnteros {
	...
	public void rango (int a, int b){
		...
		while(aux.next != null){
			if(aux.info > a && aux.info < b){ // se cumple (15 años)
				prev = prev.next;
				aux = aux.next;
			} else { //no se cumple
				aux = aux.next;

...
public class RecursividadVariosResuelto {
	 ...
	 * @return b^e
	 *
	 * Para este ejercicio utilizaremos la propia definición del exponente 
	...
	*/
	public static int potencia(int b, int e){
		if(e < 0){//¡como estamos usando variables de tipo INT, no podemos hacer potencias de números negativos!
			return 0;
		}
		...
		if(e == 0){//caso base
			...
			return b * potencia(b,e-1);//Volvemos a llamar al método realizando la operacion 
		}
	}
	 ...
	 * 
	 * Utilizaremos el método .charAt(s.length()-1) para ir recogiendo las ULTIMAS letras
	...
	public static String invertir(String s){
		if(s.length()== 0){//Caso base
			...
			return s.charAt(s.length()-1) + invertir(s.substring(0, s.length()-1));//Volvemos a llamar al método concatenando a la inversa 
		}
	}
	
	
	/**
	 * 3) Division: desarrollar el método division para que realice una
	 ...
	 * 
	 * Para este ejercicio utilizaremos la definición de división, de forma que:
	 *    8/2 == 8 - 2 = 4 - 2 = 2 - 2 = 0 - 2
	...
	public static int arrayProducto(int[] v1, int[] v2, int len){
		if(len < 0|| v1.length < len || v2.length < len){//Excepción
			return 0;
		}
		
		if(len == 0){//caso base
			return v1[0] * v2[0]; //producto de los 1º valores
		} else {
